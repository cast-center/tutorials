OncoSigRF <- function(Network_matrix_df, Gold_Standard_in_Network_names, 
                      Fraction_Gold_sample = NULL, ntrees = NULL, 
                      max_iterations = NULL, balance = NULL, to_save = NULL) {
        # This function runs the Random Forest Learner using the Network Provided and the gold standard provided

        if (is.null(Fraction_Gold_sample)) {
                Fraction_Gold_sample <- .5
        }
        if (is.null(ntrees)) {
                ntrees <- 50
        }
        ...
        for (i in 1:max_iterations) {
                ...
                importance <- as.data.frame(result$importance)
        ...
        if (to_save == 1) {
                save(all_forests, file = "All_forests.RData")
        }
